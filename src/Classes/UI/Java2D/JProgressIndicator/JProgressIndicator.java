package Classes.UI.Java2D.JProgressIndicator;

import Classes.UI.Java2D.JPanelSlider.JPanelSlider;
...
public class JProgressIndicator<E> extends JComponent implements Accessible {
    ...
    public void initSlider(JPanelSlider slider) {
        panelSlider = slider;
        slider.addEventSliderAnimatorChanged((JPanelSlider.SliderType type, float f) -> {
            if (type == JPanelSlider.SliderType.RIGHT_TO_LEFT) {
        ...
        slider.showSlid(slider.getSliderComponent()[getProgressIndex()], JPanelSlider.SliderType.NONE);
    }
    ...
    public void previous() {
        if (panelSlider != null) {
            if (panelSlider.isSlidAble()) {
                if (getProgress() > 0) {
                    panelSlider.showSlid(panelSlider.getSliderComponent()[getProgressIndex() - 1], JPanelSlider.SliderType.LEFT_TO_RIGHT);
                }
            }
        }
    }
        ...
        if (panelSlider != null) {
            if (panelSlider.isSlidAble()) {
                if (getProgress() < getModel().getSize() - 1) {
                    panelSlider.showSlid(panelSlider.getSliderComponent()[getProgressIndex() + 1], JPanelSlider.SliderType.RIGHT_TO_LEFT);
                }
            }
        }
    }
