package Classes.UI.Java2D.JGlassPanelPopUp;
...
public class JGlassPanePopup {
    protected JLayeredPane getLayerPane() {
        return layerPane;
    }

    private static JGlassPanePopup instance;
    private JLayeredPane layerPane;

    private JGlassPanePopup() {
        init();
    }
    ...
    public void addAndShowPopup(Component component, Option option, String name) {
        JPopup popup = new JPopup(this, component, option);
        if (name != null) {
            popup.setName(name);
        }
    ...
    public static void closePopup(int index) {
        ...
        if (index >= 0 && index < instance.getLayerPane().getComponentCount()) {
            if (instance.getLayerPane().getComponent(index) instanceof JPopup) {
                JPopup popup = (JPopup) instance.getLayerPane().getComponent(index);
    ...
    public static void closePopup(String name) {
        for (Component com : instance.layerPane.getComponents()) {
            if (com.getName() != null && com.getName().equals(name)) {
                if (com instanceof JPopup) {
                    JPopup popup = (JPopup) com;
                    popup.setShowPopup(false);
                }
            }
        }
